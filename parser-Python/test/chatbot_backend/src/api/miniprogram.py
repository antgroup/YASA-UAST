...
DBTABLE_KNOWLEDGE_BASE = os.getenv('DBTABLE_MINIPROGRAM_KNOWLEDGE_BASE')
DBTABLE_QUESTION_LOG = os.getenv('DBTABLE_MINIPROGRAM_LOG_QUESTIONS')
DBTABLE_QUESTION_PROCESSING_LOG = os.getenv('DBTABLE_MINIPROGRAM_QUESTION_PROCESSING_LOG')
DBTABLE_QUESTION_FEEDBACK = os.getenv('DBTABLE_MINIPROGRAM_FEEDBACK')
...
bp = Blueprint('miniprogram', __name__, url_prefix='/')


# request params: question, chat_id, role, product, version
@bp.route("/api/mini/v2/question", methods=["POST"])
@require_token
@validate_input
def index_v2(username, email):
    ...
    role, product, version = app_utils.set_role_product_version(
        request, app, version_list, is_mini=True)
    ...
    data = app_service.question_index_v2(username, email, start_time, role, product, version, chat_id,
                                         ...
                                         db_tables=MINI_DB_TABLES, es_mode=es_mode, model=model,
                                         product_line=MINI_PROGRAM_PRODUCT_LINE)
    return data, 200


@bp.route("/api/mini/v2/questionStream", methods=["POST"])
@require_token
@validate_input
def index_v2_stream(username, email):
    ...
    detected_products = app_utils.visiting_url_and_product_detected(request, app, question_original,
                                                                    product, username, chat_id)
    data = app_service.question_stream_index_v2(username, email, start_time, role, product, version, chat_id,
                                                detected_products, question_original, db_tables=MINI_DB_TABLES,
                                                es_mode=es_mode,
                                                model=model, product_line=MINI_PROGRAM_PRODUCT_LINE)
    return data


@bp.after_request
def log_mini_question_for_analysis(response):
    """
        Logs the question and answer for analysis.
        """
    # only log if the endpoint is the main endpoint
    if request.endpoint in ['miniprogram.index_v2', 'miniprogram.index_v2_stream']:
...
# Feedback
@bp.route('/api/mini/v2/feedback', methods=['POST'])
@require_token
@validate_input
