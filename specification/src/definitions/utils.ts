...
export const NODE_COMMENTS: Record<string, string> = {};
export const NODE_FIELD_COMMENTS: Record<string, Record<string, string>> = {};
...
const validFieldKeys = ["default", "optional", "validate", "comment"];
...
export default function defineType(type: string, opts: DefineTypeOpts = {}) {
  ...
  // builder决定初始化函数的入参，visitor表示AST遍历时需要递归处理的子节点属性
  ...
  for (const key of Object.keys(fields)) {
    ...
    NODE_FIELD_COMMENTS[type][key] = field.comment || '';
  ...
  NODE_COMMENTS[type] = opts.comment = comment;
